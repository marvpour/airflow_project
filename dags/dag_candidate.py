...
def create_tables(**kwargs) -> None:
    ...
    [con.execute(table) for table in [QUERY_JOB_TABLE_CREATION, QUERY_JOB_RESULT_TABLE_CREATION]]
...
def insert_recs(**kwargs) -> None:
    ...
    query = f"""WITH rows AS (INSERT INTO {JOB_TABLE_NAME} DEFAULT VALUES RETURNING id)
        INSERT INTO {JOB_RESULT_TABLE_NAME} (job_id) 
            SELECT rows.id AS job_id
            FROM rows
            ON CONFLICT (job_id)
            DO NOTHING
            """
    ...
    job_id = conn.execute(f"SELECT id FROM {JOB_TABLE_NAME}").fetchone()[0]
...
def get_hit_count(
    ...
    # I assume we dont need to keep the duplicated random numbers, so I will use a dictionary to keep counts for rach digit.
    random_numbers = [str(get_rand_digit()) for i in range(rand_digit_amount)]
    hit_count = sum([1 for number in random_numbers if number in num_str])

    task_instance = kwargs['ti']
    task_instance.xcom_push(key="hit_count", value=hit_count)
...
def branching(**kwargs) -> List[str]:
    ...
    hit_count = kwargs['ti'].xcom_pull(key='hit_count')
    return TASK_ID_ACTION_ON_GT_THRESHOLD if hit_count > HIT_COUNT_THRESHOLD else TASK_ID_ACTION_ON_LTE_THRESHOLD
...
def action_on_gt_threshold(**kwargs) -> None:
    ...
    job_table_query = f"UPDATE {JOB_TABLE_NAME} SET is_active = FALSE WHERE id={job_id};"
    job_result_table_query = f"UPDATE {JOB_RESULT_TABLE_NAME} SET is_successful = TRUE, is_gt_th = TRUE WHERE job_id = {job_id};"

    conn = get_engine().connect()
    conn.execute(text(job_table_query).execution_options(autocommit=True))
    conn.execute(text(job_result_table_query).execution_options(autocommit=True))
...
def action_on_lte_threshold(**kwargs) -> None:
    ...
    job_table_query = f"UPDATE {JOB_TABLE_NAME} SET is_active = FALSE WHERE id={job_id};"
    job_result_table_query = f"UPDATE {JOB_RESULT_TABLE_NAME} SET is_successful = TRUE, is_gt_th = FALSE WHERE job_id = {job_id};"
...
def action_on_error(**kwargs) -> None:
    ...
    job_table_query = f"UPDATE {JOB_TABLE_NAME} SET is_active = FALSE WHERE id={job_id};"
    job_result_table_query = f"UPDATE {JOB_RESULT_TABLE_NAME} SET is_successful = FALSE WHERE job_id = {job_id};"
